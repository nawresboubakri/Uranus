...
function ajouter_commande($commandes){
    $requette = "INSERT INTO commandes(ref_prod,username,categorie,prix_total,jour_arrivee)
     VALUES (:ref_prod,:username,:categorie,:prix_total,:jour_arrivee)";
...
function modifiercomm($commandes,$ref_cmd){
    $sql="UPDATE commandes SET  ref_prod=:ref_prod,username=:username,categorie=:categorie,prix_total=:prix_total,jour_arrivee=:jour_arrivee  where ref_cmd= :ref_cmd ";
...
try{		
    ...
    $datas = array(':ref_prod'=>$ref_prod,':username'=>$username,':categorie'=>$categorie,':prix_total'=>$prix_total,':jour_arrivee'=>$jour_arrivee);
    $req->bindValue(':ref_cmd',$ref_cmd);
    $req->bindValue(':ref_prod',$ref_prod);
    $req->bindValue(':username',$username);
    $req->bindValue(':categorie',$categorie);
...
class commliC{
    function afficherComsli(){
        //$sql="SElECT * From utilisateurs ";
        $sql="SELECT  * From commandes INNER JOIN livraison WHERE commandes.ref_cmd = livraison.ref_cmd";
        $db = config::getConnexion();
        try{
        $liste=$db->query($sql);
        return $liste;
