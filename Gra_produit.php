...
$listeliste=$livraison1F->afficherlis();
foreach($listeliste as $row){
if($row['destination']=='sfax'){
    $i++;
}
else if($row['destination']=='tunis'){
    $j++;
}else
if($row['destination']=='sousse'){
    $l++;
}
 ...
 $dataPoints = array(
     array("label"=> "sfax", "y"=>$i ),
     array("label"=> "tunis", "y"=> $l),
     array("label"=> "sousse", "y"=> $j),
...
var chart = new CanvasJS.Chart("chartContainer", {
	...
	exportEnabled: true,
	title:{
		text: "Les destination"
	},
	subtitles: [{
		text: "Les plus utilisers"
	}],
	data: [{
		type: "pie",
		showInLegend: "true",
...
Morris.Bar({
 ...
 xkey:'nom_prod',
 //ykeys:['quantite'],
 ykeys:['ref_prod', 'quantite', 'prix_prod'],
 labels:['ref_prod', 'quantite', 'prix_prod'],
 hideHover:'auto',
 stacked:true
});
</script>